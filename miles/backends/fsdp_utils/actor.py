...
from ...utils.memory_utils import print_memory
...
class FSDPTrainRayActor(TrainRayActor):
    ...
    def train(self, rollout_id: int, rollout_data_ref: Box) -> None:
        ...
        for mbs_id, packed_batch in enumerate(packed_batches):
            print_memory(f"FSDP-train::step {mbs_id=}")
            self._train_step(
    ...
    def _train_step(self, packed_batch, reported_accum, mbs_id, grad_accum, world_size):
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            logits = self.model(
