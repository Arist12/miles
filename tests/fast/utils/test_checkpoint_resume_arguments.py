...
def test_native_lora_checkpoint_with_custom_data_source_is_weight_only(tmp_path):
    adapter = tmp_path / "run" / "iter_0000007" / "adapter"
    adapter.mkdir(parents=True)
    args = _args(tmp_path, lora_adapter_path=str(adapter), rollout_global_dataset=False)
...
def test_non_checkpoint_adapter_path_is_always_weight_only(tmp_path):
    ...
    adapter.mkdir()
    for rollout_global_dataset in (True, False):
        args = _args(
            tmp_path,
