...
Nothing else in the tree passes ``--lora-adapter-path``, so the save/resume pair -- the whole
point of multi-stage LoRA RL -- has no coverage. Both ways it can fail are silent: the adapter
loads cleanly and the job keeps running either way, only with a policy that is garbage or is the
...
register_rocm_ci(est_time=700, suite="stage-c-4-gpu-mi350", labels=["lora"])
...
def _assert_adapter_is_trainable(adapter_dir: str, *, stage: str) -> dict:
    shards = sorted(glob.glob(os.path.join(adapter_dir, "adapter_megatron_rank*.pt")))
...
def _trained_component_norm(adapter: dict, keys: list[str]) -> float:
    """Norm over ``keys``, which name the adapter's zero-initialized side.

    ``lora_B`` starts at exactly zero, so its norm is everything training produced. The
    whole-adapter norm is not usable for this: ``lora_A``'s random init dominates it and it
    barely moves however training goes.
    """
...
def execute():
    ...
    _assert_adapter_is_trainable(stage_a_adapter, stage="stage A")
    ...
    # The load-side assertion. "Finite and moving" is also true of a run that quietly threw the
    # checkpoint away and started over, which is the failure mode this test exists for, so
    # compare where stage B started against where stage A stopped: one optimizer step apart, not
    # a fresh lora_A / zero lora_B.
    stage_a = _assert_adapter_is_trainable(stage_a_adapter, stage="stage A")
    zero_init = [k for k in stage_a if k in first and stage_a[k].float().norm() < 1.0]
    assert zero_init, "no zero-initialized adapter tensors found; the lora_B heuristic broke"
    before = _trained_component_norm(stage_a, zero_init)
    after = _trained_component_norm(first, zero_init)
